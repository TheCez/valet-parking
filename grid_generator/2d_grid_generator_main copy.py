...
def create_2d_grid(world, grid_size=500, cell_size=1):
    # Create an empty grid with 0s (free space)
    ...
    for bb in level_bbs:
        ...
        # Fill the rotated bounding box as a polygon with 1s (obstacles)
        cv2.fillPoly(grid, [np.array(grid_corners, dtype=np.int32)], color=1)
